...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch = User::where('id_user', $request['id'])->first();
        }
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('users',  function ($join) {
            $join->on('messages.from_id', '=', 'users.id_user')
                ->orOn('messages.to_id', '=', 'users.id_user');
        })
        ...
        if ($users->count() > 0) {
            ...
            foreach ($users as $user) {
                if ($user->id_user != Auth::id()) {
                    // Get user data
                    $userCollection = User::where('id_user', $user->id_user)->first();
    ...
    {
        // Get user data
        $userCollection = User::where('id_user', $request['user_id'])->first();
    ...
    public function getFavorites(Request $request)
    {
        ...
        foreach ($favorites->get() as $favorite) {
            // get user data
            $user = User::where('id_user', $favorite->favorite_id)->first();
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('nama_lengkap', 'LIKE', "%{$input}%");
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? User::where('id_user', Auth::id())->update(['dark_mode' => 1])  // Make Dark
                : User::where('id_user', Auth::id())->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    $update = User::where('id_user', Auth::id())->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        $update = $request['status'] > 0
            ? User::where('id_user', $request['user_id'])->update(['active_status' => 1])
            : User::where('id_user', $request['user_id'])->update(['active_status' => 0]);
        // send the response
        return Response::json([
            'status' => $update,
        ], 200);
